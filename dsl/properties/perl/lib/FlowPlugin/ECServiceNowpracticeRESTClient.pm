...
package FlowPlugin::ECServiceNowpracticeRESTClient;
use strict;
use warnings;
use LWP::UserAgent;
use JSON;
...
use FlowPDF::Log;
## DO NOT EDIT THIS BLOCK === rest client imports ends, checksum: d27c62dc86b12403403adc9006599e3b ===
# Place for the custom user imports, e.g. use File::Spec
# Added by Phil Cherry for uploadAttachement methods
use LWP::UserAgent;
use HTTP::Request::Common;
## DO NOT EDIT THIS BLOCK === rest client starts ===
=head1

FlowPlugin::ECServiceNowpracticeRESTClient->new('http://endpoint', {
...
use constant {
    ...
    USER_AGENT => 'ECServiceNowpracticeRESTClient REST Client',
...
Use this method to create a new FlowPlugin::ECServiceNowpracticeRESTClient, e.g.

    my $client = FlowPlugin::ECServiceNowpracticeRESTClient->new($endpoint,
...
sub getChangeRequest {
    ...
    $self->{method} = 'getChangeRequest';
    $self->{methodParameters} = \%params;

    my $uri = "/api/sn_chg_rest/change/normal?sysparm_query=sys_id={{cr_sysid}}";
...
sub createChangeRequest {
    ...
    $self->{method} = 'createChangeRequest';
    $self->{methodParameters} = \%params;

    my $uri = "/api/sn_chg_rest/change/normal";
    logDebug "URI Template: $uri";
    $uri = renderOneLineTemplate($uri, %params);
    logDebug "Rendered URI: $uri";
...
sub modifyChangeRequest {
    ...
    $self->{method} = 'modifyChangeRequest';
    $self->{methodParameters} = \%params;

    my $uri = "/api/sn_chg_rest/change/normal/{{cr_sysid}}";
    logDebug "URI Template: $uri";
    $uri = renderOneLineTemplate($uri, %params);
    logDebug "Rendered URI: $uri";
...
# table_sys_id: in body

# uploadFile: in body

sub uploadAttachementToChangeRequest {
    my ($self, %params) = @_;

    $self->{method} = 'uploadAttachementToChangeRequest';

    my $uri = "/api/now/attachment/upload";
    my $endpoint = $self->{endpoint};
    $endpoint =~ s{/+$}{};
    my ($web, $host) = split '://', $endpoint;

    my $auth = $self->{basicAuth};
    my $username = $auth->{userName};
    my $password = $auth->{password};

    my $lwpself = LWP::UserAgent->new();
    $lwpself->timeout(10);
   ...
   my $req = POST $url, Content_Type => 'multipart/form-data',
    ...
    my $response = $lwpself->request($req);

    return $response;
}

## DO NOT EDIT THIS BLOCK === rest client ends, checksum: 4be63376ed2f4533e0611dbe8c93af28 ===
=pod
